...
## 画像表示ルール
話題に関連する画像を見せたい場合は返答の中に [IMAGE:カテゴリ/ファイル名] を含めること（1回の返答につき最大1枚）。
「これ見て！」「이 사진 봐요！」「この画像どう思う？」のように自然に会話に織り交ぜること。
[VOCAB]ブロックの後に置くこと。

利用可能な画像（実際に存在するファイルのみ使うこと）：
- 料理: food/bibimbap.jpg, food/fried_chicken.jpg, food/tteokbokki.jpg, food/korean_bbq.jpg, food/cheese_dakgalbi.jpg, food/street_food.jpg
- 旅行: travel/gyeongbokgung.jpg, travel/myeongdong.jpg, travel/jeju.jpg, travel/namsan.jpg, travel/bukchon.jpg, travel/han_river.jpg
- スポーツ: sports/baseball.jpg, sports/soccer.jpg, sports/basketball.jpg
- K-pop: kpop/kpop_concert.jpg, kpop/stage_lights.jpg, kpop/microphone.jpg
- ゲーム: game/blue_archive.jpg, game/blue_archive2.jpg, game/genshin.jpg, game/genshin2.jpg
- アニメ: anime/kimetsu.jpg, anime/kimetsu2.jpg, anime/jujutsu.jpg, anime/jujutsu2.jpg

画像を使う場面の例：
- 料理の話をしている → bibimbap.jpg や tteokbokki.jpg を見せる
- 旅行の話 → namsan.jpg や myeongdong.jpg を見せる
- K-popの話 → kpop_concert.jpg を見せる
- ゲームの話 → blue_archive.jpg や genshin.jpg を見せる
- アニメの話 → kimetsu.jpg や jujutsu.jpg を見せる
画像がない話題（挨拶・文法練習など）は省略してよい。`;
...
export default function App() {
  ...
  const [imageLightbox,  setImageLightbox]  = useState(null); // null | "/images/xxx"
    ...
    try {
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
          ...
          const image   = extractImage(data.text);
          setMessages(prev => [...prev, { role: "assistant", content: korean, ja, showJA: false, words, image }]);
                      ...
                      {m.image && (
                        <img
                          ...
                          style={{
                            ...
                            boxShadow: '0 2px 8px rgba(232,93,107,.15)',
                          }}
                          onError={e => { e.currentTarget.style.display = 'none'; }}
                          onClick={e => {
                            e.stopPropagation();
                            setImageLightbox(`/images/${m.image}`);
                          }}
                        />
                      )}
                      {m.showJA && m.ja && (
